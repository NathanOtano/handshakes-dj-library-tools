...
param(
    ...
    [switch] $KeepStale,
    [switch] $Apply,
    [switch] $RepairManagedPlaylistIds,
    [switch] $SyncManagedPlaylistXml,
    [switch] $IncludePlans,
...
$RuntimeRoot = if ($RuntimeRoot) { $RuntimeRoot } else { Join-Path $repoRoot 'runtime\rekordbox-genre-taxonomy-sync' }
$TaxonomyPath = if ($TaxonomyPath) { $TaxonomyPath } else { Join-Path $repoRoot 'config\dj-genre-taxonomy.json' }
$runtimeFullPath = [System.IO.Path]::GetFullPath($RuntimeRoot)
$helperPath = [System.IO.Path]::GetFullPath((Join-Path $scriptRoot 'rekordbox_genre_taxonomy_sync.py'))
...
function Copy-RekordboxDatabaseSet {
    ...
    $destination = Assert-UnderRepoRuntime -Path $DestinationRoot
    New-Item -ItemType Directory -Path $destination -Force | Out-Null
    foreach ($name in @('master.db', 'master.db-wal', 'master.db-shm', 'masterPlaylists6.xml', 'automixPlaylist6.xml')) {
...
function Invoke-TaxonomyHelper {
    param(
    ...
    if ($KeepStale) {
        $arguments += '--keep-stale'
    }
    if ($HelperApply) {
        $arguments += '--apply'
    }
    if ($RepairManagedPlaylistIds) {
        $arguments += '--repair-managed-playlist-ids'
    }
    if ($SyncManagedPlaylistXml) {
        $arguments += '--sync-managed-playlist-xml'
    }
    if ($IncludePlans) {
        $arguments += '--include-plans'
    }
    ...
    try {
        ...
        $modeMatch = [regex]::Match($payload, '\{\s*"mode"\s*:')
        $start = if ($modeMatch.Success) { $modeMatch.Index } else { $payload.IndexOf('{') }
        $end = $payload.LastIndexOf('}')
        if ($start -lt 0 -or $end -le $start) {
            throw
        }
        $jsonPayload = $payload.Substring($start, $end - $start + 1)
        $compactJson = $jsonPayload | & $PythonPath -c "import json, sys; print(json.dumps(json.load(sys.stdin), ensure_ascii=False))"
        $parsed = ($compactJson | Out-String) | ConvertFrom-Json
    }
...
if ($Mode -eq 'Plan' -or ($Mode -eq 'LiveApply' -and -not $Apply)) {
    ...
    $helper = Invoke-TaxonomyHelper -PythonPath $pythonPath -DatabasePath $masterCopy -DatabaseDir $planRoot
    if (-not $KeepRuntime) {
...
if ($Mode -eq 'CopyApply') {
    ...
    $helper = Invoke-TaxonomyHelper -PythonPath $pythonPath -DatabasePath $masterCopy -DatabaseDir $copyRoot -HelperApply -AllowRunningCommit
    if (-not $KeepRuntime) {
...
$backupManifest = [pscustomobject]@{
    ...
    purpose = 'pre-live-genre-taxonomy-sync-backup'
    rekordboxRoot = $RekordboxRoot
    taxonomyPath = $taxonomyFullPath
    keepStale = [bool] $KeepStale
    copied = $backup
}
$backupManifest | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath $backupManifestPath -Encoding UTF8

$helperLive = Invoke-TaxonomyHelper -PythonPath $pythonPath -DatabasePath $masterPath -DatabaseDir $RekordboxRoot -HelperApply
