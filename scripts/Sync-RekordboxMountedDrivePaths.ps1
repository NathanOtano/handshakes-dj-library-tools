...
param(
    ...
    [string] $RuntimeRoot = (Join-Path $PSScriptRoot '..\runtime\rekordbox-mounted-drive-relink'),
    [string] $SourcePrefix = 'D:/DJ_Music',
    [string] $TargetPrefix = 'C:/DJ_Music',
...
$helperPath = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'rekordbox_mounted_drive_relink.py'))
...
function Invoke-RelinkHelper {
    ...
    [pscustomobject]@{ exitCode = $exit; payload = $parsed }
}
...
if ($Mode -eq 'Plan' -or ($Mode -eq 'LiveApply' -and -not $Apply)) {
    ...
    $helper = Invoke-RelinkHelper -PythonPath $pythonPath -DatabasePath $masterCopy -DatabaseDir $planRoot
    if (-not $KeepRuntime) {
...
if ($Mode -eq 'CopyApply') {
    ...
    $helper = Invoke-RelinkHelper -PythonPath $pythonPath -DatabasePath $masterCopy -DatabaseDir $copyRoot -HelperApply
    if (-not $KeepRuntime) {
...
$backupManifest = [pscustomobject]@{
    ...
    purpose = 'pre-live-rekordbox-mounted-drive-relink-backup'
    rekordboxRoot = $RekordboxRoot
...
$helperLive = Invoke-RelinkHelper -PythonPath $pythonPath -DatabasePath $masterPath -DatabaseDir $RekordboxRoot -HelperApply
